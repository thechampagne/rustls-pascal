 ...
 *)
type
   rustls_client_cert_verifier		= record
					  end;
 ...
 *)
type
   rustls_client_cert_verifier_optional	= record
					  end;
 ...
 *)
type
   rustls_client_config_builder		= record
					  end;

type
   rustls_connection			= record
					  end;
 ...
 *)
type
   rustls_server_config_builder		= record
					  end;
 ...
 *)
type
   rustls_slice_slice_bytes		= record
					  end;
 ...
 *)
type
   rustls_supported_ciphersuite		= record
					  end;

type
   Pchar  = ^char;
   Prustls_certificate  = ^rustls_certificate;
   Prustls_certified_key  = ^rustls_certified_key;
   Prustls_client_cert_verifier  = ^rustls_client_cert_verifier;
   Prustls_client_cert_verifier_optional  = ^rustls_client_cert_verifier_optional;
   Prustls_client_config  = ^rustls_client_config;
   Prustls_client_config_builder  = ^rustls_client_config_builder;
   Prustls_connection  = ^rustls_connection;
   Prustls_iovec  = ^rustls_iovec;
   Prustls_root_cert_store  = ^rustls_root_cert_store;
   Prustls_server_config  = ^rustls_server_config;
   Prustls_server_config_builder  = ^rustls_server_config_builder;
   Prustls_slice_slice_bytes  = ^rustls_slice_slice_bytes;
   Prustls_slice_str  = ^rustls_slice_str;
   Prustls_supported_ciphersuite  = ^rustls_supported_ciphersuite;
   Pcsize_t  = ^csize_t;
   Puint16  = ^cuint16;
   Puint8  = ^cuint8;

type
   rustls_result = (RUSTLS_RESULT_OK := 7000,RUSTLS_RESULT_IO := 7001,
		    RUSTLS_RESULT_NULL_PARAMETER := 7002,RUSTLS_RESULT_INVALID_DNS_NAME_ERROR := 7003,
		    RUSTLS_RESULT_PANIC := 7004,RUSTLS_RESULT_CERTIFICATE_PARSE_ERROR := 7005,
		    RUSTLS_RESULT_PRIVATE_KEY_PARSE_ERROR := 7006,
		    RUSTLS_RESULT_INSUFFICIENT_SIZE := 7007,
		    RUSTLS_RESULT_NOT_FOUND := 7008,RUSTLS_RESULT_INVALID_PARAMETER := 7009,
		    RUSTLS_RESULT_UNEXPECTED_EOF := 7010,RUSTLS_RESULT_PLAINTEXT_EMPTY := 7011,
		    RUSTLS_RESULT_CORRUPT_MESSAGE := 7100,
		    RUSTLS_RESULT_NO_CERTIFICATES_PRESENTED := 7101,
		    RUSTLS_RESULT_DECRYPT_ERROR := 7102,RUSTLS_RESULT_FAILED_TO_GET_CURRENT_TIME := 7103,
 ...
 *)
type
   rustls_tls_version = (RUSTLS_TLS_VERSION_SSLV2 := 512,RUSTLS_TLS_VERSION_SSLV3 := 768,
 ...
 *)
type
   Prustls_slice_bytes  = ^rustls_slice_bytes;
   rustls_slice_bytes = record
			   data : ^cuint8;
			   len : csize_t;
			end;
(**
 ...
 *)
type
   Prustls_verify_server_cert_params  = ^rustls_verify_server_cert_params;
   rustls_verify_server_cert_params = record
...
type
   rustls_verify_server_cert_callback = function (userdata:rustls_verify_server_cert_user_data; params:Prustls_verify_server_cert_params):cuint32;cdecl;

type
   rustls_log_level = csize_t;

type
   Prustls_log_params  = ^rustls_log_params;
   rustls_log_params = record
			  level : rustls_log_level;
			  message : rustls_str;
		       end;

type
   rustls_log_callback = procedure (userdata:pointer; params:Prustls_log_params);cdecl;
 ...
 *)
type
   rustls_read_callback = function (userdata:pointer; buf:Puint8; n:csize_t; out_n:Pcsize_t):rustls_io_result;cdecl;
 ...
 *)
type
   rustls_write_callback = function (userdata:pointer; buf:Puint8; n:csize_t; out_n:Pcsize_t):rustls_io_result;cdecl;
 ...
 *)
type
   rustls_write_vectored_callback = function (userdata:pointer; iov:Prustls_iovec; count:csize_t; out_n:Pcsize_t):rustls_io_result;cdecl;
 ...
 *)
type
   Prustls_client_hello  = ^rustls_client_hello;
   rustls_client_hello = record
			    sni_name : rustls_str;
 ...
 *)
type
   rustls_client_hello_callback = function (userdata:rustls_client_hello_userdata; hello:Prustls_client_hello):Prustls_certified_key;cdecl;
 ...
 * `userdata` will be supplied based on rustls_client,server_session_set_userdata.
 *
 ...
 *)
type
   rustls_session_store_get_callback = function (userdata:rustls_session_store_userdata; key:Prustls_slice_bytes; remove_after:longint; buf:Puint8; count:csize_t; 
						 out_n:Pcsize_t):cuint32;cdecl;
 ...
 *)
type
   rustls_session_store_put_callback = function (userdata:rustls_session_store_userdata; key:Prustls_slice_bytes; val:Prustls_slice_bytes):cuint32;cdecl;


var
   RUSTLS_ALL_CIPHER_SUITES : array[0..8] of Prustls_supported_ciphersuite;cvar;external;
   RUSTLS_ALL_CIPHER_SUITES_LEN : csize_t;cvar;external;
   RUSTLS_DEFAULT_CIPHER_SUITES : array[0..8] of Prustls_supported_ciphersuite;cvar;external;
   RUSTLS_DEFAULT_CIPHER_SUITES_LEN : csize_t;cvar;external;
   RUSTLS_ALL_VERSIONS : array[0..1] of cuint16;cvar;external;
   RUSTLS_ALL_VERSIONS_LEN : csize_t;cvar;external;
   RUSTLS_DEFAULT_VERSIONS : array[0..1] of cuint16;cvar;external;
   RUSTLS_DEFAULT_VERSIONS_LEN : csize_t;cvar;external;
 ...
 *)
function rustls_version:rustls_str; cdecl; external;

(**
 * Get the DER data of the certificate itself.
 * The data is owned by the certificate and has the same lifetime.
 *)
function rustls_certificate_get_der(cert:Prustls_certificate; out_der_data:Puint8; out_der_len:Pcsize_t):rustls_result; cdecl; external;
 ...
 *)
function rustls_supported_ciphersuite_get_suite(supported_ciphersuite:Prustls_supported_ciphersuite):cuint16; cdecl; external;
 ...
 * need to be freed.
 *)
function rustls_supported_ciphersuite_get_name(supported_ciphersuite:Prustls_supported_ciphersuite):rustls_str; cdecl; external;

(**
 * Return the length of rustls' list of supported cipher suites.
 *)
function rustls_all_ciphersuites_len:csize_t; cdecl; external;
 ...
 *)
function rustls_all_ciphersuites_get_entry(i: csize_t):Prustls_supported_ciphersuite; cdecl; external;

(**
 * Return the length of rustls' list of default cipher suites.
 *)
function rustls_default_ciphersuites_len:csize_t; cdecl; external;
 ...
 * building a ClientConfig or ServerConfig.
 *)
function rustls_default_ciphersuites_get_entry(i:csize_t):Prustls_supported_ciphersuite; cdecl; external;
 ...
 *)
function rustls_certified_key_build(cert_chain:Puint8; cert_chain_len:csize_t; private_key:Puint8; private_key_len:csize_t; certified_key_out:Prustls_certified_key):rustls_result; cdecl; external;
 ...
 *)
function rustls_certified_key_get_certificate(certified_key:Prustls_certified_key; i:csize_t):Prustls_certificate; cdecl; external;
 ...
 * by the application.
 *)
function rustls_certified_key_clone_with_ocsp(certified_key:Prustls_certified_key; ocsp_response:Prustls_slice_bytes; cloned_key_out:Prustls_certified_key):rustls_result; cdecl; external;
 ...
 *)
procedure rustls_certified_key_free(key:Prustls_certified_key); cdecl; external;
 ...
 *)
function rustls_root_cert_store_new:Prustls_root_cert_store; cdecl; external;
 ...
 * certificates.
 *)
function rustls_root_cert_store_add_pem(store:Prustls_root_cert_store; pem:Puint8; pem_len:csize_t; strict:boolean):rustls_result; cdecl; external;

(**
 * Free a rustls_root_cert_store previously returned from rustls_root_cert_store_builder_build.
 * Calling with NULL is fine. Must not be called twice with the same value.
 *)
procedure rustls_root_cert_store_free(store:Prustls_root_cert_store); cdecl; external;
 ...
 *)
function rustls_client_cert_verifier_new(store:Prustls_root_cert_store):Prustls_client_cert_verifier; cdecl; external;
 ...
 * Calling with NULL is fine. Must not be called twice with the same value.
 *)
procedure rustls_client_cert_verifier_free(verifier:Prustls_client_cert_verifier); cdecl; external;
 ...
 *)
function rustls_client_cert_verifier_optional_new(store:Prustls_root_cert_store):Prustls_client_cert_verifier_optional; cdecl; external;
 ...
 * Calling with NULL is fine. Must not be called twice with the same value.
 *)
procedure rustls_client_cert_verifier_optional_free(verifier:Prustls_client_cert_verifier_optional); cdecl; external;
 ...
 *)
function rustls_client_config_builder_new:Prustls_client_config_builder; cdecl; external;
 ...
 * ownership. `len` is the number of consecutive `cuint16` pointed to by `versions`.
 *)
function rustls_client_config_builder_new_custom(cipher_suites:Prustls_supported_ciphersuite; cipher_suites_len:csize_t; tls_versions:Puint16; tls_versions_len:csize_t; builder_out:Prustls_client_config_builder):rustls_result; cdecl; external;
 ...
 *)
function rustls_client_config_builder_dangerous_set_certificate_verifier(config_builder:Prustls_client_config_builder; callback:rustls_verify_server_cert_callback):rustls_result; cdecl; external;
 ...
 *)
function rustls_client_config_builder_use_roots(config_builder:Prustls_client_config_builder; roots:Prustls_root_cert_store):rustls_result; cdecl; external;

(**
 * Add trusted root certificates from the named file, which should contain
 * PEM-formatted certificates.
 *)
function rustls_client_config_builder_load_roots_from_file(config_builder:Prustls_client_config_builder; filename:Pchar):rustls_result; cdecl; external;
 ...
 *)
function rustls_client_config_builder_set_alpn_protocols(builder:Prustls_client_config_builder; protocols:Prustls_slice_bytes; len:csize_t):rustls_result; cdecl; external;

(**
 * Enable or disable SNI.
 * <https://docs.rs/rustls/0.20.0/rustls/struct.ClientConfig.html#structfield.enable_sni>
 *)
procedure rustls_client_config_builder_set_enable_sni(config:Prustls_client_config_builder; enable:boolean); cdecl; external;
 ...
 *)
function rustls_client_config_builder_set_certified_key(builder:Prustls_client_config_builder; certified_keys:Prustls_certified_key; certified_keys_len:csize_t):rustls_result; cdecl; external;

(**
 * Turn a *rustls_client_config_builder (mutable) into a const *rustls_client_config
 * (read-only).
 *)
function rustls_client_config_builder_build(builder:Prustls_client_config_builder):Prustls_client_config; cdecl; external;
 ...
 * was created.
 *)
procedure rustls_client_config_builder_free(config:Prustls_client_config_builder); cdecl; external;
 ...
 * Calling with NULL is fine. Must not be called twice with the same value.
 *)
procedure rustls_client_config_free(config:Prustls_client_config); cdecl; external;
 ...
 *)
function rustls_client_connection_new(config:Prustls_client_config; hostname:Pchar; conn_out:Prustls_connection):rustls_result; cdecl; external;
 ...
 *)
procedure rustls_connection_set_userdata(conn:Prustls_connection; userdata:pointer); cdecl; external;
 ...
 *)
procedure rustls_connection_set_log_callback(conn:Prustls_connection; cb:rustls_log_callback); cdecl; external;
 ...
 *)
function rustls_connection_read_tls(conn:Prustls_connection; callback:rustls_read_callback; userdata:pointer; out_n:Pcsize_t):rustls_io_result; cdecl; external;
 ...
 *)
function rustls_connection_write_tls(conn:Prustls_connection; callback:rustls_write_callback; userdata:pointer; out_n:Pcsize_t):rustls_io_result; cdecl; external;
 ...
 *)
function rustls_connection_write_tls_vectored(conn:Prustls_connection; callback:rustls_write_vectored_callback; userdata:pointer; out_n:Pcsize_t):rustls_io_result; cdecl; external;
 ...
 *)
function rustls_connection_process_new_packets(conn:Prustls_connection):rustls_result; cdecl; external;

(**
 * <https://docs.rs/rustls/0.20.0/rustls/struct.CommonState.html#method.wants_read>
 *)
function rustls_connection_wants_read(conn:Prustls_connection):boolean; cdecl; external;

(**
 * <https://docs.rs/rustls/0.20.0/rustls/struct.CommonState.html#method.wants_write>
 *)
function rustls_connection_wants_write(conn:Prustls_connection):boolean; cdecl; external;

(**
 * <https://docs.rs/rustls/0.20.0/rustls/struct.CommonState.html#method.is_handshaking>
 *)
function rustls_connection_is_handshaking(conn:Prustls_connection):boolean; cdecl; external;
 ...
 *)
procedure rustls_connection_set_buffer_limit(conn:Prustls_connection; n:csize_t); cdecl; external;
 ...
 *)
procedure rustls_connection_send_close_notify(conn:Prustls_connection); cdecl; external;
 ...
 * first argument has type `rustls_connection *` (as opposed to
 *  `const rustls_connection *`).
 * <https://docs.rs/rustls/0.20.0/rustls/enum.Connection.html#method.peer_certificates>
 *)
function rustls_connection_get_peer_certificate(conn:Prustls_connection; i:csize_t):Prustls_certificate; cdecl; external;
 ...
 *)
procedure rustls_connection_get_alpn_protocol(conn:Prustls_connection; protocol_out:Puint8; protocol_out_len:Pcsize_t); cdecl; external;
 ...
 *)
function rustls_connection_get_protocol_version(conn:Prustls_connection):cuint16; cdecl; external;
 ...
 *)
function rustls_connection_get_negotiated_ciphersuite(conn:Prustls_connection):Prustls_supported_ciphersuite; cdecl; external;
 ...
 *)
function rustls_connection_write(conn:Prustls_connection; buf:Puint8; count:csize_t; out_n:Pcsize_t):rustls_result; cdecl; external;
 ...
 *)
function rustls_connection_read(conn:Prustls_connection; buf:Puint8; count:csize_t; out_n:Pcsize_t):rustls_result; cdecl; external;

{$ifdef DEFINE_READ_BUF}
 ...
 *)
function rustls_connection_read_2(conn:Prustls_connection; buf:Puint8; count:csize_t; out_n:Pcsize_t):rustls_result; cdecl; external;
 ...
 *)
procedure rustls_connection_free(conn:Prustls_connection); cdecl; external;
 ...
 *)
procedure rustls_error(result:dword; buf:Pchar; len:csize_t; out_n:Pcsize_t); cdecl; external;

function rustls_result_is_cert_error(result:dword):boolean; cdecl; external;

(**
 * Return a rustls_str containing the stringified version of a log level.
 *)
function rustls_log_level_str(level:rustls_log_level):rustls_str; cdecl; external;

(**
 * Return the length of the outer slice. If the input pointer is NULL,
 * returns 0.
 *)
function rustls_slice_slice_bytes_len(input:Prustls_slice_slice_bytes):csize_t; cdecl; external;
 ...
 * rustls_slice_slice_bytes, returns rustls_slice_bytesNULL, 0.
 *)
function rustls_slice_slice_bytes_get(input:Prustls_slice_slice_bytes; n:csize_t):rustls_slice_bytes; cdecl; external;

(**
 * Return the length of the outer slice. If the input pointer is NULL,
 * returns 0.
 *)
function rustls_slice_str_len(input:Prustls_slice_str):csize_t; cdecl; external;

(**
 * Retrieve the nth element from the input slice of `&str`s. If the input
 * pointer is NULL, or n is greater than the length of the
 * rustls_slice_str, returns rustls_strNULL, 0.
 *)
function rustls_slice_str_get(input:Prustls_slice_str; n:csize_t):rustls_str; cdecl; external;
 ...
 * for the cipher suites, key exchange groups and protocol versions.
 *)
function rustls_server_config_builder_new:Prustls_server_config_builder; cdecl; external;
 ...
 * ownership. `len` is the number of consecutive `cuint16` pointed to by `versions`.
 *)
function rustls_server_config_builder_new_custom(cipher_suites:Prustls_supported_ciphersuite; cipher_suites_len:csize_t; tls_versions:Puint16; tls_versions_len:csize_t; builder_out:Prustls_server_config_builder):rustls_result; cdecl; external;
 ...
 *)
procedure rustls_server_config_builder_set_client_verifier(builder:Prustls_server_config_builder; verifier:Prustls_client_cert_verifier); cdecl; external;
 ...
 * For memory lifetime, see rustls_server_config_builder_new.
 *)
procedure rustls_server_config_builder_set_client_verifier_optional(builder:Prustls_server_config_builder; verifier:Prustls_client_cert_verifier_optional); cdecl; external;
 ...
 * was created.
 *)
procedure rustls_server_config_builder_free(config:Prustls_server_config_builder); cdecl; external;
 ...
 *)
function rustls_server_config_builder_set_ignore_client_order(builder:Prustls_server_config_builder; ignore:boolean):rustls_result; cdecl; external;
 ...
 *)
function rustls_server_config_builder_set_alpn_protocols(builder:Prustls_server_config_builder; protocols:Prustls_slice_bytes; len:csize_t):rustls_result; cdecl; external;
 ...
 * configured certified keys and vice versa.
 *)
function rustls_server_config_builder_set_certified_keys(builder:Prustls_server_config_builder; certified_keys:Prustls_certified_key; certified_keys_len:csize_t):rustls_result; cdecl; external;

(**
 * Turn a *rustls_server_config_builder (mutable) into a const *rustls_server_config
 * (read-only).
 *)
function rustls_server_config_builder_build(builder:Prustls_server_config_builder):Prustls_server_config; cdecl; external;
 ...
 * Calling with NULL is fine. Must not be called twice with the same value.
 *)
procedure rustls_server_config_free(config:Prustls_server_config); cdecl; external;
 ...
 *)
function rustls_server_connection_new(config:Prustls_server_config; conn_out:Prustls_connection):rustls_result; cdecl; external;
 ...
 *)
function rustls_server_connection_get_sni_hostname(conn:Prustls_connection; buf:Puint8; count:csize_t; out_n:Pcsize_t):rustls_result; cdecl; external;
 ...
 *)
function rustls_server_config_builder_set_hello_callback(builder:Prustls_server_config_builder; callback:rustls_client_hello_callback):rustls_result; cdecl; external;
 ...
 *)
function rustls_client_hello_select_certified_key(hello:Prustls_client_hello; certified_keys:Prustls_certified_key; certified_keys_len:csize_t; out_key:Prustls_certified_key):rustls_result; cdecl; external;
 ...
 *)
function rustls_server_config_builder_set_persistence(builder:Prustls_server_config_builder; get_cb:rustls_session_store_get_callback; put_cb:rustls_session_store_put_callback):rustls_result; cdecl; external;
